...
**WebUSB — 動作確認済み**。ベンダー固有クラスであるため、Chrome から
ドライバのシムなしで直接アクセスできる。WinUSB も Zadig も不要。単一の
自己完結型 HTML ファイルでハードウェアタイムコード表示器が**実際に動く**。
macOS 上で本機に対して実装・動作確認済み。本プロジェクトが必要になった理由で
ある「macOS にドライバが無いこと」が、そのままブラウザ経路を容易にしている。

実装する場合に知っておくべき点が3つある。

- `selectAlternateInterface(0, 1)` が必要。理由は「罠 1」を参照。
- 「罠 2」のウォームアップも必要。これを省くと最初のレジスタ読み出しが
  空パケットを返し、接続失敗に見える。
- **`file://` で十分**。Chrome は `file://` を potentially trustworthy な
  オリジンとして扱うため、サーバー無しで WebUSB が使える。ただし許可は
  オリジンごとに記憶されるので `file://` と `http://localhost:8000` は別扱いに
  なる。また**一度許可したデバイスは選択ダイアログに出なくなる**。再度
  プロンプトを出すのではなく `navigator.usb.getDevices()` で許可済みハンドルを
  再利用すること。さもないとアプリが壊れたように見える。
